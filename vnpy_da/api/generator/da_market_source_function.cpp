bool MarketApi::reqUserLogin(const dict &req, int reqid)
{
...
bool MarketApi::reqBrokerData(const dict &req, int reqid)
{
	CMarketReqBrokerDataField myreq = CMarketReqBrokerDataField();
	memset(&myreq, 0, sizeof(myreq));
	getString(req, "ContCode", myreq.ContCode);
	getString(req, "ErrorDescription", myreq.ErrorDescription);
	bool i = this->api->ReqBrokerData(&myreq, reqid);
	return i;
};

bool MarketApi::reqMarketData(const dict &req, int reqid)
{
	...
	getString(req, "ErrorDescription", myreq.ErrorDescription);
	bool i = this->api->ReqMarketData(&myreq, reqid);
	return i;
};
